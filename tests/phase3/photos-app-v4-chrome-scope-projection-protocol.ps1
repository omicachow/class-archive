...
Assert-True ($runner.Contains("new URL('/class-archive-core/login', settings.photos)") -and $runner.Contains('page.waitForURL') -and $runner.Contains('home_projection_') -and $server.Contains("['/class-archive-core/login', '/identification.php?redirect=%252Findex.php%253F%252Fclass-archive-photo-app']")) 'v4_scope_authenticated_bridge_and_home_projection_readiness_missing'
