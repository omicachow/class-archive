...
const dom = await readFile(files.dom, 'utf8');
const searchOverlay = await readFile(files.searchOverlay, 'utf8');
...
const navigationMatch = app.match(/const navigation = Object\.freeze\(\[(?<items>[\s\S]*?)\]\);/);
check(navigationMatch?.groups?.items, 'primary navigation must be declared as a bounded owned list');
const primaryNavigation = [...navigationMatch.groups.items.matchAll(/href:\s*'([^']+)'/g)].map((match) => match[1]);
assert.deepEqual(primaryNavigation, ['/home', '/photos'], 'primary navigation must contain only Collections and Library');
assertions += 1;
check(i18n.includes("'nav.home': '精选集'") && i18n.includes("'nav.photos': '资料库'"),
  'primary navigation labels must use the frozen Collections and Library product names');
for (const route of ['/home', '/photos', '/people', '/search', '/albums', '/memories', '/my']) {
  check(server.includes(`'${route}'`), `BFF must preserve the compatible owned route ${route}`);
}
for (const nonPrimaryRoute of ['/people', '/search', '/albums', '/memories', '/my']) {
  check(!primaryNavigation.includes(nonPrimaryRoute), `${nonPrimaryRoute} must not remain a primary navigation destination`);
}
...
check(server.includes('const photoUiStaticManifest = Object.freeze([')
  && server.includes("['/photo-ui/ui-dom.js', 'ui-dom.js']")
  && server.includes("['/photo-ui/ui-search-overlay.js', 'ui-search-overlay.js']")
  && server.includes('const photoUiStaticPaths = new Map(photoUiStaticManifest);'),
  'BFF must use an explicit static-resource manifest including every owned UI helper module');
check(server.includes('const photoUiAssetNames = Object.freeze(photoUiStaticManifest.map(([, fileName]) => fileName));')
  && server.includes('const names = photoUiAssetNames;'),
  'all allowlisted owned modules must contribute to the immutable asset revision');
check(app.includes("from './ui-dom.js?v=__PHOTO_UI_ASSET_REV__'")
  && app.includes("from './ui-search-overlay.js?v=__PHOTO_UI_ASSET_REV__'")
  && dom.includes("import { t } from './i18n.js?v=__PHOTO_UI_ASSET_REV__';")
  && searchOverlay.includes("from './i18n.js?v=__PHOTO_UI_ASSET_REV__';")
  && searchOverlay.includes("from './ui-dom.js?v=__PHOTO_UI_ASSET_REV__';"),
  'owned UI helpers must use the same immutable revisioned module graph');
for (const helper of ['element', 'append', 'emptyState', 'loadingState', 'errorState', 'toast', 'dialogShell', 'labeledControl', 'labeledGroup', 'option']) {
  check(dom.includes(`export function ${helper}(`), `owned DOM module must export ${helper}`);
  check(!app.includes(`function ${helper}(`), `application module must import ${helper} instead of retaining a duplicate`);
}
check(dom.includes('let dialogSequence = 0;')
  && dom.includes('dialog-title-${dialogSequence}')
  && !dom.includes('runtime.')
  && !dom.includes('fetch(')
  && !dom.includes('/api/'),
  'owned DOM helpers must remain presentation-only and independent of runtime or API state');
check(searchOverlay.includes('export function openGlobalSearchOverlay(')
  ...
  && searchOverlay.includes("dialog.dataset.searchOverlay = 'true'")
  ...
  && searchOverlay.includes('returnFocus.focus('),
  'global search must use an owned native modal dialog with a recoverable focus return');
check(!searchOverlay.includes('fetch(') && !searchOverlay.includes('/api/') && !searchOverlay.includes('runtime.'),
  'global search shell must remain presentation-only while the application owns policy-bound transport');
for (const framework of ['react', 'preact', 'svelte', 'vue', 'solid-js', 'lit-html']) {
  check(!new RegExp(`(?:from\\s+['\"]|import\\s*\\(|require\\s*\\()(?:(?!\\n).)*${framework}`, 'i').test(`${app}\n${dom}\n${searchOverlay}`),
    `owned UI must not introduce a ${framework} framework dependency`);
}
...
check(server.includes('function photoUiDocumentQueryAllowed(url)')
  && server.includes("url.pathname === '/home'")
  && server.includes("url.searchParams.has('search')")
  && server.includes("url.pathname !== '/search'"),
  'owned routes must fail closed while allowing only the bounded global-search compatibility state');
...
check(app.includes("apiJson(`/api/class-archive/search/grouped?${request.params}`"),
  'search must use the one typed Class Archive grouped projection');
check(!app.includes("apiJson('/api/search/metadata'") && !app.includes("apiJson('/api/search/smart'"),
  'search must not bypass the typed Gateway through legacy upstream calls');
check(app.includes('function normalizeGroupedSearch(payload, expectedScope)')
  && app.includes('function mergeGroupedSearchPages(current, next)'),
  'search must validate the grouped result shape and use Gateway-bound continuation cursors');
check(app.includes('function openSearchFromTrigger(')
  ...
  && app.includes('history.back()'),
  'Search must open as a global overlay whose browser Back action closes the overlay before leaving the page');
check(app.includes("event.key.toLowerCase() === 'k'")
  && app.includes("event.key === '/'")
  && app.includes('event.ctrlKey || event.metaKey'),
  'global search must support Ctrl/Cmd+K and slash shortcuts without introducing a framework router');
check(app.includes("'/home?search=1'") || app.includes('searchParams.set(\'search\', \'1\')'),
  'legacy /search must redirect or canonicalize into the global search overlay state');
check(app.includes('runtime.currentSearchContext')
  && searchOverlay.includes('dataset.scopeToggle')
  && app.includes('function supportsScopedSuggestions(scope)')
  && app.includes('function groupedSearch(query, scope, cursor = null, signal = undefined)'),
  'global search must bind current-context scope to persistent suggestions and the typed grouped-search contract');
const groupedRenderStart = app.indexOf('function renderGroupedSearchResults(response, onQuery, onLoadMore = null)');
const structuredRenderIndex = app.indexOf('if (structuredCount > 0 || response.photos.items.length > 0)', groupedRenderStart);
const smartRenderIndex = app.indexOf('if (response.semantic.available && response.semantic.items.length > 0)', groupedRenderStart);
check(groupedRenderStart >= 0 && structuredRenderIndex > groupedRenderStart && smartRenderIndex > structuredRenderIndex,
  'global search must render structured archive results before optional semantic Beta matches');
check(!searchOverlay.includes('emptyState(') && !searchOverlay.includes('search.initialTitle') && !searchOverlay.includes('search.initialBody'),
  'global search initial state must stay lightweight rather than render a legacy large empty-state card');
for (const key of [
  ...
  'search.suggestionBasketball', 'search.discoveryHint', 'search.overlayTitle', 'search.scopeAll',
]) check(i18n.includes(`'${key}'`), `search discovery copy must be centralized for ${key}`);
check(css.includes('.global-search-dialog') && css.includes('.global-search-surface')
  && css.includes('.global-search-input') && css.includes('.global-search-results'),
  'global search must have owned dialog styles rather than reusing an administrative page layout');
...
check(app.includes('searchWithin.addEventListener')
  && app.includes('runtime.currentSearchContext')
  && app.includes("setSearchContext({ kind: 'ALBUM', id: album.id"),
  'album detail must open bounded current-album global search rather than navigate to a permanent search page');
check(app.includes("normalizeGroupedSearchSection('dates', payload.archiveTime"),
...
check(app.includes("const MOBILE_NAVIGATION = new Set(['photos', 'home'])")
  && app.includes('mobile-search-action')
  && app.includes('dataset.globalSearchTrigger'),
  'mobile navigation must expose exactly Library, Collections, and the global search action');
check(css.includes('grid-template-columns: repeat(3, minmax(0, 1fr))'), 'mobile navigation must have three equal primary actions');
...
check(css.includes('padding-bottom: env(safe-area-inset-bottom)') || css.includes('env(safe-area-inset-bottom)'),
  'mobile global search and navigation must honor the bottom safe area');
...
check(app.includes("dataset.avatarMenuTrigger = 'true'")
  ...
  'the account avatar must expose the non-primary identity menu with native button semantics');
check(app.includes("reducedMotion ? 'auto' : 'smooth'"),
  'archive navigation must not force smooth motion for reduced-motion users');
