...
async function assertLayout(page, mobile, code, requireMobileNavigation = true) {
  ...
  if (mobile && requireMobileNavigation) {
    assert(result.mobileVisible && result.mobileLinks === 3, `${code}_mobile_navigation`);
...
async function waitForGlobalSearchOverlay(page, role) {
  ...
  assert(await dialog.count() === 1, `${role}_global_search_overlay`);
  const input = dialog.getByRole('combobox', { name: '搜索照片', exact: true });
  assert(await input.count() === 1, `${role}_global_search_input`);
  await input.waitFor({ state: 'visible', timeout: 30_000 }).catch(() => null);
  assert(await input.evaluate((element) => document.activeElement === element), `${role}_global_search_focus`);
  return { dialog, input };
}
...
async function searchSuggestionsJourney(page, role) {
  ...
  const { input } = await waitForGlobalSearchOverlay(page, role);
  const responsePromise = page.waitForResponse((response) => {
  ...
  if (href) {
    ...
    await page.locator('.global-search-status').waitFor({ state: 'visible', timeout: 30_000 }).catch(() => null);
    assert(await page.locator('.global-search-status').count() === 1, `${role}_search_suggestions_result_status`);
  }
  ...
  await goto(page, photoOrigin, '/home?search=1', `${role}_search_after_suggestions`);
  await waitForGlobalSearchOverlay(page, role);
}

async function searchPage(page, role, mobile) {
  await goto(page, photoOrigin, '/home?search=1', `${role}_search`);
  await waitForGlobalSearchOverlay(page, role);
  ...
  if (environment === 'synthetic') {
    await page.locator('.global-search-status').filter({ hasText: '部分搜索能力暂时不可用' }).waitFor({ state: 'visible', timeout: 30_000 }).catch(() => null);
    assert(await page.locator('.photo-card').count() === 0, `${role}_search_safe_empty_results`);
    assert(await page.locator('.global-search-status').getByText('部分搜索能力暂时不可用，已显示安全可确认的结果。', { exact: true }).count() === 1
      || (await page.locator('.global-search-status').innerText()).includes('部分搜索能力暂时不可用'), `${role}_search_safe_partial_copy`);
  ...
  assert(!/暂时不可用/.test(await page.locator('.global-search-status').innerText()), `${role}_search_partial`);
...
async function leafAlbumDetail(page, role, mobile) {
  ...
  assert(await within.count() === 1, `${role}_leaf_album_search_context_link`);
  await assertLayout(page, mobile, `${role}_leaf_album`);
  await assertBusinessCopy(page, `${role}_leaf_album`);
  await Promise.all([
    page.waitForURL(/\/home\?search=1$/i, { timeout: 30_000 }).catch(() => null),
    within.click(),
  ]);
  assert(/^\/home$/.test(new URL(page.url()).pathname) && new URL(page.url()).searchParams.get('search') === '1', `${role}_album_search_context_route`);
  const { dialog } = await waitForGlobalSearchOverlay(page, role);
  assert(await dialog.locator('.global-search-context').count() === 1, `${role}_album_search_context_visible`);
}
