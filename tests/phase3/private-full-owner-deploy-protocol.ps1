...
# 8291 staging endpoint or from bypassing the exact v15→v16 rollback snapshot.
...
# takes a snapshot; exact v16 explicitly skips it. Piwigo alone stops for the
...
Assert-True ($deploy.Contains('$migrationSourceVersion = 15') -and $deploy.Contains('$migrationTargetVersion = 16')) 'private_full_deploy_v15_to_v16_contract_missing'
...
Assert-True ($snapshot.Contains('15:16')) 'private_full_snapshot_v15_to_v16_gate_missing'
...
# remain distinguishable from the two accepted v15→v16 states.
$syntheticRequired = 'PRE_MIGRATION_DB_SNAPSHOT=REQUIRED_CURRENT_V15 schema_current=15 schema_from=15 schema_to=16 scope=DB_ONLY media=NOT_INCLUDED'
$syntheticCurrent = 'PRE_MIGRATION_DB_SNAPSHOT=NOT_REQUIRED_CURRENT_V16 schema_current=16 schema_from=16 schema_to=16 scope=NONE media=NOT_INCLUDED'
foreach ($case in @(
    ...
    @{ record = 'PRE_MIGRATION_DB_SNAPSHOT=REQUIRED_CURRENT_V15 schema_current=15 schema_from=15 schema_to=17 scope=DB_ONLY media=NOT_INCLUDED'; accepted = $false },
    @{ record = 'PRE_MIGRATION_DB_SNAPSHOT=NOT_REQUIRED_CURRENT_V16 schema_current=15 schema_from=16 schema_to=16 scope=NONE media=NOT_INCLUDED'; accepted = $false },
