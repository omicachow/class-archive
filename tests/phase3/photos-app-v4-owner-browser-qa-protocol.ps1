...
Assert-Contains $wrapper '.codex-work\private-real-qa\runtime\photos-app-v4-owner-guest\opaque-media-probes' 'owner_fqa_guest_probe_root_missing'
...
Assert-Contains $wrapper "Write-FqaBrokerControl -Broker `$Broker -Command ('EXPORT ' + `$Run)" 'owner_fqa_broker_export_command_missing'
Assert-Contains $wrapper '$Broker.StandardInput.BaseStream' 'owner_fqa_broker_direct_control_stream_missing'
Assert-NotContains $wrapper '.StandardInput.WriteLine(' 'owner_fqa_streamwriter_control_forbidden'
...
Assert-Contains $runner 'GUEST_MEDIA_PROBE_SCOPE' 'owner_fqa_guest_probe_scope_missing'
Assert-Contains $runner 'writeGuestOpaqueMediaProbe' 'owner_fqa_guest_probe_writer_missing'
Assert-Contains $runner "fs.openSync(guestMediaProbeOutput, 'wx', 0o600)" 'owner_fqa_guest_probe_exclusive_write_missing'
Assert-Contains $runner 'fs.fsyncSync(descriptor)' 'owner_fqa_guest_probe_durable_write_missing'
Assert-Contains $runner 'guest_media_probe_authorized_' 'owner_fqa_guest_probe_authorized_media_guard_missing'
...
Assert-Contains $runner "import { createHash } from 'node:crypto';" 'owner_fqa_runner_fqt_hash_import_missing'
Assert-Contains $runner 'const FQT_DISCLOSURE_FINGERPRINTS = Object.freeze(new Set([' 'owner_fqa_runner_fqt_fingerprint_set_missing'
Assert-Contains $runner 'function assertNoFqtDisclosure(value, code)' 'owner_fqa_runner_fqt_fail_closed_assertion_missing'
Assert-Contains $runner 'async function assertClassmateFqtDom(page, code)' 'owner_fqa_runner_fqt_html_dom_audit_missing'
Assert-Contains $runner 'async function assertClassmateFqtReadBoundary(page)' 'owner_fqa_runner_fqt_read_boundary_missing'
Assert-Contains $runner 'if (fqtDisclosureAudit) assertNoFqtDisclosure(result.text, `${code}_fqt_api`);' 'owner_fqa_runner_fqt_normal_api_scan_missing'
Assert-Contains $runner 'if (fqtDisclosureAudit) assertNoFqtDisclosure(result?.text ??' 'owner_fqa_runner_fqt_semantic_api_scan_missing'
Assert-Contains $runner '`${role}_semantic_fqt_api`' 'owner_fqa_runner_fqt_semantic_api_code_missing'
Assert-Contains $runner "stageAt('classmate_no_discovery')" 'owner_fqa_runner_fqt_runtime_stage_missing'
Assert-Contains $runner 'await assertClassmateFqtReadBoundary(classmateSession.page);' 'owner_fqa_runner_fqt_runtime_boundary_call_missing'
Assert-Contains $runner "['/api/class-archive/manage/people', 'manage_people']" 'owner_fqa_runner_fqt_manage_people_denial_missing'
Assert-Contains $runner "new URL('/class-archive-core/identity', photoOrigin)" 'owner_fqa_runner_fqt_core_identity_surface_missing'
Assert-True (-not [regex]::IsMatch($runner, '(?i)fqa[-_]t[-_][a-z0-9]{4,}')) 'owner_fqa_runner_fqt_target_literal_forbidden'
Assert-True (-not [regex]::IsMatch($runner, '(?i)fqa_[a-f0-9]{12,}_teacher')) 'owner_fqa_runner_fqt_account_literal_forbidden'
...
Assert-Contains $runner 'const reachedViewer = page.waitForURL' 'owner_fqa_viewer_route_settlement_missing'
Assert-Contains $runner "value.pathname === target.pathname && value.search === ''" 'owner_fqa_viewer_exact_target_guard_missing'
Assert-Contains $runner 'const historyRestored = page.waitForURL' 'owner_fqa_search_history_settlement_missing'
Assert-Contains $runner "value.pathname === '/home' && value.search === ''" 'owner_fqa_search_history_exact_home_guard_missing'
Assert-Contains $runner 'search_history_restored' 'owner_fqa_search_history_assertion_missing'
...
Assert-Contains $wrapper '[string]$GuestMediaProbeDocument' 'owner_fqa_optional_guest_probe_parameter_missing'
Assert-Contains $wrapper 'Resolve-FqaGuestMediaProbeDocument' 'owner_fqa_guest_probe_path_binding_missing'
Assert-Contains $wrapper 'Assert-FqaGuestMediaProbeDocument' 'owner_fqa_guest_probe_acl_handoff_missing'
Assert-Contains $wrapper 'Set-ClassArchiveOwnerOnlyFileAcl -Path $guestMediaProbePath' 'owner_fqa_guest_probe_exact_acl_before_handoff_missing'
Assert-Contains $wrapper "'CLASS_ARCHIVE_V4_OWNER_GUEST_MEDIA_PROBE_OUTPUT'" 'owner_fqa_guest_probe_environment_missing'
Assert-Contains $wrapper '$preserveGuestMediaProbe' 'owner_fqa_guest_probe_cleanup_gate_missing'
Assert-Contains $wrapper "-Code 'guest_media_probe_cleanup'" 'owner_fqa_guest_probe_failure_cleanup_missing'
