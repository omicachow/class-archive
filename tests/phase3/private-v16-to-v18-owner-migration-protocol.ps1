...
Assert-True ($snapshotAction.Contains("`$snapshotRecoveryPending=`$true") -and $snapshotAction.Contains('Enter-Maintenance') -and $snapshotAction.Contains('finally') -and $snapshotAction.Contains('Restore-SnapshotOwnerAvailability') -and $snapshotAction.Contains('Finalize-SnapshotRecoveryMaintenance; Assert-OwnerRuntime') -and $snapshotAction.Contains("`$snapshotRecoveryPending=`$false")) 'private_v16_to_v18_snapshot_failure_recovery_finally_missing'
