...
Assert-True ($snapshotAvailabilityRecoveryFunction.Contains('Ensure-SnapshotWriterForRecovery') -and $snapshotAvailabilityRecoveryFunction.Contains("if (`$maintenance -eq 'ACTIVE') { Finalize-SnapshotRecoveryMaintenance }") -and $snapshotAvailabilityRecoveryFunction.Contains('Assert-OwnerRuntime') -and $snapshotAvailabilityRecoveryFunction.Contains('snapshot_recovery_maintenance_state_unknown')) 'private_v16_to_v18_snapshot_availability_recovery_missing'
...
Assert-True ($adapter.Contains("'attest-v16-to-v18-synthetic-direct-runtime.ps1'") -and $adapter.Contains('function Invoke-DirectV16ToV18ProofGate') -and $adapter.Contains("@('-Action','verify')") -and $adapter.Contains('attempt=attempt36') -and $adapter.Contains('direct_runtime_proof_gate_missing') -and $adapter.Contains('direct_runtime_proof_gate_head_stale')) 'private_v16_to_v18_direct_runtime_gate_invocation_missing'
