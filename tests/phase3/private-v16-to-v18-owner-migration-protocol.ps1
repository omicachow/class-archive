...
Assert-True ($snapshotMaintenanceStateFunction.Contains("'CLASS_ARCHIVE_STATUS:503'") -and $snapshotMaintenanceStateFunction.Contains("'Class Archive maintenance mode.'") -and $snapshotMaintenanceStateFunction.Contains("'--noproxy','*'") -and $snapshotMaintenanceStateFunction.Contains("'http://127.0.0.1:8190/'") -and $snapshotMaintenanceStateFunction.Contains('Invoke-ClassArchiveBoundedNative') -and $snapshotMaintenanceStateFunction.Contains("return 'INACTIVE'") -and $snapshotMaintenanceStateFunction.Contains("return 'UNKNOWN'")) 'private_v16_to_v18_snapshot_maintenance_state_fail_closed_missing'
Assert-True (-not $snapshotMaintenanceStateFunction.Contains("Invoke-Piwigo @('ps','-q','piwigo')") -and -not $snapshotMaintenanceStateFunction.Contains("'docker','exec',`$containerId,'curl'") -and -not $snapshotMaintenanceStateFunction.Contains('class_archive_private_full_v3_piwigo-piwigo-1')) 'private_v16_to_v18_snapshot_maintenance_probe_must_use_loopback_boundary'
Assert-True ($waitMaintenanceFunction.Contains('Get-SnapshotMaintenanceState') -and $waitMaintenanceFunction.Contains("-eq 'ACTIVE'") -and -not $waitMaintenanceFunction.Contains("Invoke-Piwigo @('exec','-T','piwigo','curl'")) 'private_v16_to_v18_wait_maintenance_loopback_probe_missing'
Assert-True ($snapshotRecoveryFinalizerFunction.Contains("CLASS_ARCHIVE_OWNER_V16_SNAPSHOT_RECOVERY=1") -and $snapshotRecoveryFinalizerFunction.Contains('finalize-owner-v16-snapshot-recovery.php') -and $snapshotRecoveryFinalizerFunction.Contains("'--finalize-owner-v16-snapshot-recovery'") -and $snapshotRecoveryFinalizerFunction.Contains("'--user','nginx'")) 'private_v16_to_v18_snapshot_v16_finalizer_not_narrowly_bound'
Assert-True ($snapshotWriterRecoveryFunction.Contains("if ((Get-SnapshotMaintenanceState) -eq 'ACTIVE') { return }") -and $snapshotWriterRecoveryFunction.Contains("Invoke-Piwigo @('up','-d','--no-deps','piwigo')") -and $snapshotWriterRecoveryFunction.Contains("if (`$maintenance -eq 'INACTIVE')") -and $snapshotWriterRecoveryFunction.Contains('Enter-Maintenance') -and $snapshotWriterRecoveryFunction.Contains('snapshot_recovery_maintenance_not_ready') -and $snapshotWriterRecoveryFunction.Contains('AddSeconds(180)')) 'private_v16_to_v18_snapshot_writer_recovery_missing'
Assert-True (-not $snapshotWriterRecoveryFunction.Contains('--force-recreate') -and -not $snapshotWriterRecoveryFunction.Contains('class_archive_private_full_v3_piwigo-piwigo-1')) 'private_v16_to_v18_snapshot_writer_recovery_must_not_race_fixed_container'
$snapshotCreateFunction = Slice-Function $adapter 'function Create-PreMigrationSnapshot' 'function Get-SnapshotBinding' 'private_v16_to_v18_snapshot_create_function_boundary_missing'
Assert-True ($snapshotCreateFunction.Contains("Set-SnapshotStage 'STOP_WRITER'") -and $snapshotCreateFunction.Contains("Set-SnapshotStage 'CAPTURE_BASELINE'") -and $snapshotCreateFunction.Contains("Set-SnapshotStage 'CAPTURE_DB_SNAPSHOT'") -and -not $snapshotCreateFunction.Contains('finally') -and -not $snapshotCreateFunction.Contains('RecreatePiwigoUnderMaintenance')) 'private_v16_to_v18_snapshot_single_recovery_path_missing'
Assert-True ($snapshotAvailabilityRecoveryFunction.Contains('Ensure-SnapshotWriterForRecovery') -and $snapshotAvailabilityRecoveryFunction.Contains("if (`$maintenance -eq 'ACTIVE')") -and $snapshotAvailabilityRecoveryFunction.Contains('Finalize-SnapshotRecoveryMaintenance') -and $snapshotAvailabilityRecoveryFunction.Contains('Assert-OwnerRuntime') -and $snapshotAvailabilityRecoveryFunction.Contains('snapshot_recovery_maintenance_state_unknown') -and $snapshotAvailabilityRecoveryFunction.Contains('snapshotRecoveryStage')) 'private_v16_to_v18_snapshot_availability_recovery_missing'
...
Assert-True ($snapshotAction.Contains("`$snapshotRecoveryPending=`$true") -and $snapshotAction.Contains("Set-SnapshotStage 'ENTER_MAINTENANCE'") -and $snapshotAction.Contains('Create-PreMigrationSnapshot') -and $snapshotAction.Contains('Ensure-SnapshotWriterForRecovery') -and $snapshotAction.Contains("Set-SnapshotStage 'BIND_SNAPSHOT'") -and $snapshotAction.Contains('finally') -and $snapshotAction.Contains('Restore-SnapshotOwnerAvailability') -and $snapshotAction.Contains('Get-V16ToV18FailureCode') -and $snapshotAction.Contains('snapshot_primary_') -and $snapshotAction.Contains('Finalize-SnapshotRecoveryMaintenance; Assert-OwnerRuntime') -and $snapshotAction.Contains("`$snapshotRecoveryPending=`$false")) 'private_v16_to_v18_snapshot_failure_recovery_finally_missing'
Assert-True ($adapter.Contains("`$snapshotStageSuffix=if (`$Action -eq 'Snapshot')") -and $adapter.Contains("' stage=' + [string]`$script:snapshotStage")) 'private_v16_to_v18_snapshot_failure_stage_diagnostic_missing'
Assert-True ($adapter.Contains('snapshotPrimaryStage') -and $adapter.Contains('snapshotPrimaryFailureLine') -and $adapter.Contains('snapshotRecoveryStage') -and $adapter.Contains("' primary_stage=' + [string]`$script:snapshotPrimaryStage")) 'private_v16_to_v18_snapshot_primary_diagnostic_preservation_missing'
...
Assert-True ($adapter.Contains("'attest-v16-to-v18-synthetic-direct-runtime.ps1'") -and $adapter.Contains('function Invoke-DirectV16ToV18ProofGate') -and $adapter.Contains("@('-Action','verify')") -and $adapter.Contains('attempt=attempt39') -and $adapter.Contains('direct_runtime_proof_gate_missing') -and $adapter.Contains('direct_runtime_proof_gate_head_stale')) 'private_v16_to_v18_direct_runtime_gate_invocation_missing'
