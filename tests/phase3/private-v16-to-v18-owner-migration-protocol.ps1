...
Assert-True ($snapshotMaintenanceStateFunction.Contains("Invoke-Piwigo @('ps','-q','piwigo')") -and $snapshotMaintenanceStateFunction.Contains("'docker','exec',`$containerId,'curl'") -and $snapshotMaintenanceStateFunction.Contains("^[a-f0-9]{12,64}$") -and -not $snapshotMaintenanceStateFunction.Contains('class_archive_private_full_v3_piwigo-piwigo-1')) 'private_v16_to_v18_snapshot_maintenance_probe_must_resolve_current_container'
Assert-True ($waitMaintenanceFunction.Contains('Get-SnapshotMaintenanceState') -and $waitMaintenanceFunction.Contains("-eq 'ACTIVE'") -and -not $waitMaintenanceFunction.Contains("Invoke-Piwigo @('exec','-T','piwigo','curl'")) 'private_v16_to_v18_wait_maintenance_dynamic_probe_missing'
