...
try {
    ...
    Assert-Protocol ($operator.Contains('function Save-PrivateFailureDiagnostic') -and $operator.Contains("`$diagnosticRoot 'supplemental-apply-error.json'")) 'private_failure_diagnostic_missing'
    Assert-Protocol ($operator.Contains(".codex-work\owner-restore\runtime") -and $operator.Contains(".codex-work\private-real-full\runtime")) 'private_failure_diagnostic_target_isolation_missing'
    ...
    Assert-Protocol ($operator.Contains('PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=schema schema=17 source_paths=NOT_READ')) 'operator_schema_gate_missing'
    ...
    Assert-Protocol (-not $operator.Contains("'/workspace/infra/scripts/install-class-archive-plugins.php', '--finalize-maintenance'")) 'apply_must_not_finalize_maintenance'
    Assert-Protocol (-not $operator.Contains("'/workspace/infra/scripts/rebuild-photo-read-projection.php', '--scope=all', '--json'")) 'apply_must_not_publish_projection'
    Assert-Protocol `
        ($operator.Contains("`$script:stage = 'maintenance_hold_verify'") `
            -and $operator.Contains('maintenance=HELD next=INCREMENTAL_MEDIA_THEN_EXPLICIT_FINALIZE')) `
        'maintenance_hold_handoff_missing'
    ...
    Assert-Protocol ($target.Contains("Schema::CURRENT_VERSION !== 17")) 'target_source_schema_gate_missing'
    Assert-Protocol ($target.Contains("(int) (`$migration['schema_version'] ?? 0) !== 17")) 'target_database_schema_gate_missing'
    ...
    Assert-Protocol ($target.Contains("`$sourceExisting === 0 && `$canonicalExisting > 0") -and $target.Contains('supplemental_preflight_journal_resume_invalid')) 'fresh_canonical_collision_journal_gate_missing'
    Assert-Protocol ($target.Contains('p.`piwigo_image_id`=i.`piwigo_image_id`') -and $target.Contains('$manifest[''item_presentations''][$itemDigest]')) 'journal_resume_checkpoint_binding_missing'
