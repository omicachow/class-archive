...
Assert-Contains $broker 'elseif ($cleanupSafe && $leaseOpened)' 'owner_fqa_failed_action_safe_close_attestation_missing'
Assert-Contains $broker 'CLOSED attests only that the temporary lease has been safely refrozen' 'owner_fqa_safe_close_semantics_missing'
...
Assert-Contains $wrapper "Write-FqaBrokerControl -Broker `$Process -Command ('STOP ' + `$Run) -CloseAfter" 'owner_fqa_wrapper_stop_missing'
...
Assert-Contains $wrapper 'function Write-FqaBrokerControl' 'owner_fqa_broker_control_helper_missing'
Assert-Contains $wrapper "Write-FqaBrokerControl -Broker `$Broker -Command ('EXPORT ' + `$Run)" 'owner_fqa_broker_export_command_missing'
Assert-Contains $wrapper '$Broker.StandardInput.BaseStream' 'owner_fqa_broker_control_direct_stream_missing'
Assert-Contains $wrapper '[Text.UTF8Encoding]::new($false).GetBytes($Command + "`n")' 'owner_fqa_broker_control_no_bom_bytes_missing'
...
Assert-NotContains $wrapper '$info.StandardInputEncoding' 'owner_fqa_winps_unsupported_stdin_encoding_forbidden'
Assert-Contains $wrapper 'ProcessStartInfo.StandardInputEncoding' 'owner_fqa_bom_control_comment_missing'
Assert-Contains $wrapper '$originalConsoleInputEncoding = [Console]::InputEncoding' 'owner_fqa_console_input_encoding_capture_missing'
Assert-Contains $wrapper '[Console]::InputEncoding = [Text.UTF8Encoding]::new($false)' 'owner_fqa_console_input_encoding_no_bom_missing'
Assert-Contains $wrapper 'try { [Console]::InputEncoding = $originalConsoleInputEncoding }' 'owner_fqa_console_input_encoding_restore_missing'
Assert-Contains $wrapper 'lease_broker_console_encoding_restore_recovery_failed' 'owner_fqa_console_input_encoding_restore_fail_closed_missing'
