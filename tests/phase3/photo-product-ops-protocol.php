...
foreach ($businessTables as $table) {
    ...
    $assert(str_contains($sources['contracts'], '"' . $table . '"'), "format-8 recovery contract omits {$table}");
...
$assert(str_contains($sources['contracts'], 'CA_RECOVERY_FORMAT=8') && str_contains($sources['contracts'], 'CA_RECOVERY_SCHEMA_VERSION=16'), 'historical format-8/schema-16 contract was changed');
$assert(str_contains($sources['contracts'], '"rebuildable_projection_tables":["read_projection","read_photo"]'), 'recovery contract does not classify v16 projection tables as rebuildable cache');
$assert(str_contains($sources['contracts'], '"projection_rebuild":"ALL"'), 'recovery contract does not require full projection rebuild');
$assert(str_contains($sources['contracts'], '"native_source_epoch"'), 'recovery contract omits the durable native source epoch business table');
$backupSchemaCheck = $position($sources['compose'], 'ca_recovery_select_by_schema "$${ci_version}"');
...
$assert($backupSchemaCheck >= 0 && $databaseDump >= 0 && $backupSchemaCheck < $databaseDump, 'backup must select a known versioned recovery contract before the SQL snapshot');
...
$clearTarget = $position($sources['restore'], 'clear_target "$target"');
