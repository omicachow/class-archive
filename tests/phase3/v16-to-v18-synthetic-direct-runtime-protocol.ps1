...
# Static-only contract for the attempt37 orchestration layer. It opens
...
Assert-True ($runner.Contains("`$attempt = 'attempt37'") -and $runner.Contains("`$httpPort = '11798'") -and $runner.Contains("`$compatPort = '11799'") -and $runner.Contains("`$composeProject = 'class_archive_v18_synthetic_migration_attempt37'")) 'direct_runtime_attempt37_identity_not_fixed'
...
Assert-True ($runner.Contains('if ($TimeoutSeconds -lt 1 -or $TimeoutSeconds -gt 600)')) 'direct_runtime_short_raw_probe_timeout_range_missing'
