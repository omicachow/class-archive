...
async function assertMobileLayout(page, code) {
  const layout = await page.evaluate(() => {
    ...
    const actions = nav ? [...nav.querySelectorAll('a, button')] : [];
    const minTarget = actions.length ? Math.min(...actions.map((item) => {
      const box = item.getBoundingClientRect();
      return Math.min(box.width, box.height);
    })) : 0;
    return { overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 1, visible: nav ? getComputedStyle(nav).display !== 'none' : false, links: actions.length, minTarget };
  });
  assert(!layout.overflow, `${code}_horizontal_overflow`);
  assert(layout.visible && layout.links === 3 && layout.minTarget >= 44, `${code}_mobile_navigation`);
}
...
async function browse(viewport, mobile) {
  ...
  try {
    ...
    assert(await page.getByRole('heading', { name: '精选集', exact: true }).count() >= 1, 'home_heading_missing');
    ...
    assert(await page.getByRole('heading', { name: '资料库', exact: true }).count() >= 1, 'photos_heading_missing');
    ...
    assert(Number.isInteger(sourceCounts.SOURCE_A) && sourceCounts.SOURCE_A > 0, 'source_a_leaf_albums_missing');
    assert(Number.isInteger(sourceCounts.SOURCE_B) && sourceCounts.SOURCE_B > 0, 'source_b_leaf_albums_missing');
    const filters = page.locator('.album-filter-bar[aria-label="相册来源筛选"]');
    assert(await filters.count() === 1, 'album_source_filters_missing');
    const sourceFilterLabel = (sourceKind, fallback) => {
      const labels = [...new Set(albumContract.payload.items
        .filter((album) => album.sourceKind === sourceKind)
        .map((album) => typeof album.sourceLabel === 'string' ? album.sourceLabel.trim() : '')
        .filter(Boolean))];
      return labels.length === 1 ? labels[0] : fallback;
    };
    await filters.getByRole('button', { name: sourceFilterLabel('SOURCE_A', '来源 A'), exact: true }).click();
    assert(await page.locator('.album-card').count() === sourceCounts.SOURCE_A, 'source_a_album_filter_count_invalid');
    await filters.getByRole('button', { name: sourceFilterLabel('SOURCE_B', '来源 B'), exact: true }).click();
    assert(await page.locator('.album-card').count() === sourceCounts.SOURCE_B, 'source_b_album_filter_count_invalid');
    ...
    assert(!albumSurfaceText.includes('PRIVATE_SOURCE_A'), 'raw_source_code_publicly_visible');
    ...
    const withinAlbum = page.getByRole('button', { name: '在此相册中搜索', exact: true });
    assert(await withinAlbum.count() === 1, 'leaf_album_search_link_invalid');
    ...
    const searchDialog = page.locator('dialog[data-search-overlay="true"][open]');
    await searchDialog.waitFor({ state: 'visible', timeout: 30_000 }).catch(() => null);
    assert(new URL(page.url()).pathname === '/home' && new URL(page.url()).searchParams.get('search') === '1', 'legacy_search_not_canonicalized');
    assert(await searchDialog.count() === 1, 'search_overlay_missing');
    const searchInput = searchDialog.getByRole('combobox', { name: '搜索照片', exact: true });
    assert(await searchInput.count() === 1, 'search_input_missing');
    assert(await searchInput.evaluate((input) => document.activeElement === input), 'search_input_not_focused');
    ...
    assert(await page.locator('.global-search-results[hidden]').count() === 1, 'search_results_visible_before_query');
