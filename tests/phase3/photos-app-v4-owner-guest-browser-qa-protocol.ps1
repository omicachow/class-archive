...
Assert-NotContains $runner 'raw.length >=' 'guest_opaque_media_arbitrary_minimum_length_present'
...
Assert-Contains $wrapper 'Remove-V4OwnerGuestPrivateFile' 'guest_opaque_document_cleanup_missing'
Assert-Contains $wrapper "-Code 'media_probe_document_cleanup'" 'guest_opaque_document_exact_cleanup_missing'
Assert-Contains $wrapper 'media_probe_document_cleanup_failed' 'guest_opaque_document_cleanup_failure_gate_missing'
...
Assert-Contains $wrapper 'Protect-V4OwnerGuestPrivateTree' 'guest_private_profile_cleanup_acl_normalization_missing'
Assert-Contains $wrapper '($Code + ''_tree_reparse'')' 'guest_private_profile_cleanup_reparse_fail_closed_missing'
Assert-Contains $wrapper '($Code + ''_tree_file_changed'')' 'guest_private_profile_cleanup_type_race_guard_missing'
Assert-Contains $wrapper 'Set-ClassArchiveOwnerOnlyFileAcl -Path $fileItem.FullName' 'guest_private_profile_cleanup_file_acl_normalization_missing'
...
Assert-Contains $wrapper 'Assert-ClassArchiveOwnerOnlyFileAcl -Path $browserRoot' 'guest_private_profile_root_exact_acl_missing'
Assert-Contains $wrapper 'fresh owner-only execution subtree' 'guest_stale_profile_sibling_isolation_missing'
