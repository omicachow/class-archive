...
Assert-Protocol ($source.Contains('function Copy-ContainerFileToPrivateSnapshot') -and $source.Contains('Stage to an ASCII-only') -and $source.Contains("'-d', 'Ubuntu', '--exec', 'sh', '-eu', '-c', `$stageRunner") -and $source.Contains('$psi.Arguments = ''-d Ubuntu --exec cat '' + $stageFile') -and $source.Contains('mariadb_dump_stage_hash_mismatch')) 'snapshot_ascii_relative_wsl_copy_not_array_safe'
...
Assert-Protocol ($source.Contains('function Get-OwnerOnlyDirectorySecurity') -and $source.Contains('function Set-OwnerOnlyDirectorySecurity') -and $source.Contains('$Item.GetAccessControl()') -and $source.Contains('$Item.SetAccessControl($Acl)') -and $source.Contains('$acl = Get-OwnerOnlyDirectorySecurity $item')) 'snapshot_directory_acl_noprofile_backend_missing'
