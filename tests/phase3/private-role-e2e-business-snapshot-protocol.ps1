...
Assert-Protocol ($source.Contains('function Copy-ContainerFileToPrivateSnapshot') -and $source.Contains('strictly validated ASCII relative target') -and $source.Contains("'sh', '-eu', '-c', `$copyRunner, 'sh', `$ContainerSource")) 'snapshot_ascii_relative_wsl_copy_not_array_safe'
