...
# Windows PowerShell decodes native stdout through Console.OutputEncoding.  The
# repository root includes non-ASCII characters, so leave Git output in a
# deterministic UTF-8 shape before comparing it to the resolved root.  This is
# a gate correctness requirement: damaged path bytes must never be mistaken for
# a safe public-boundary result.
$script:priorConsoleOutputEncoding = [Console]::OutputEncoding
...
function Stop-Gate([string]$Reason) {
    [Console]::OutputEncoding = $script:priorConsoleOutputEncoding
