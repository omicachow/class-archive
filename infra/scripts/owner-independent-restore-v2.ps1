...
function New-RestoreNginxConfiguration {
    ...
    # $realip_remote_addr is the original BFF peer, so the independent
    # restore address must be present in the exact map as well as the real-ip
    # trust list.  Keep this /32-only: the isolated network itself is not an
    # authorization boundary for the member-upload marker.
    $mapAnchor = '        10.241.0.10 1;'
    $realIpAnchor = '        set_real_ip_from 10.241.0.10/32;'
    Assert-RestoreV2 (([regex]::Matches($source,[regex]::Escape($mapAnchor))).Count -eq 1 -and
        ([regex]::Matches($source,[regex]::Escape($realIpAnchor))).Count -eq 1 -and
        -not $source.Contains('        10.246.0.10 1;') -and
        -not $source.Contains('set_real_ip_from 10.246.0.10/32;')) 'restore_nginx_trust_anchor_invalid'
    $generated = $source.Replace(
        $mapAnchor,
        $mapAnchor + "`n        # Independent v2 restore compatibility BFF.`n        10.246.0.10 1;"
    ).Replace(
        $realIpAnchor,
        $realIpAnchor + "`n        # Independent v2 restore compatibility BFF.`n        set_real_ip_from 10.246.0.10/32;"
    )
    Assert-RestoreV2 (([regex]::Matches($generated,'(^|\n)\s*10\.246\.0\.10 1;')).Count -eq 1 -and
        ([regex]::Matches($generated,'set_real_ip_from 10\.246\.0\.10/32;')).Count -eq 1) 'restore_nginx_generation_failed'
