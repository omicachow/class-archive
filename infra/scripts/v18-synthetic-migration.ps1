...
$attemptSpec = switch ($Attempt) {
    ...
    'attempt28' {
        # attempt27 remains preserved after an interrupted custom service wait
        # during a cold engine start. attempt28 delegates bounded health waits
        # to Docker Compose itself, retaining the direct runner's owner-only
        # child capture. It shares no project, volumes, bridges, or ports with
        # any prior laboratory.
        @{
    ...
    'attempt32' {
        # attempt31 remains preserved after a raw state probe requested a
        # timeout below its underlying bounded native helper minimum. attempt32
        # uses the unified 1..600 second bounded native timeout range and is
        # otherwise a fresh, independently initialised laboratory.
        @{
            HttpPort = '11690'; CompatPort = '11691'
            AppSubnet = '10.255.27.0/24'; GatewaySubnet = '10.198.0.0/16'
            BffGatewayIp = '10.198.0.10'
        }
    }
    'attempt33' {
        # attempt32 is preserved with a valid direct proof whose head-bound
        # attestation became stale after the owner-control timeout repair.
        # attempt33 is a fresh replacement laboratory: it never shares an
        # existing project, volume, bridge, capture directory, or loopback
        # port with that preserved evidence.
        @{
    ...
    'attempt36' {
        # attempt35 remains preserved with valid runtime evidence that became
        # head-stale after the V16 snapshot-recovery finalizer was hardened.
        # attempt36 is a newly allocated, empty direct laboratory. It shares
        # no project, volume, bridge, capture directory, or loopback port with
        # any prior attempt.
        @{
            HttpPort = '11796'; CompatPort = '11797'
            AppSubnet = '10.255.31.0/24'; GatewaySubnet = '10.193.0.0/16'
            BffGatewayIp = '10.193.0.10'
        }
    }
    'attempt37' {
        # attempt36 remains preserved with its valid proof bound to the prior
        # recovery-orchestration commit. attempt37 is a newly allocated,
        # empty direct laboratory for the current immutable source closure;
        # it shares no project, volume, bridge, capture directory, or loopback
        # port with any prior evidence.
        @{
    ...
    'attempt39' {
        # attempt38 remains preserved with its direct proof bound to the
        # pre-loopback-probe source closure. attempt39 is a fresh, empty direct
        # laboratory for the current snapshot-maintenance implementation; it
        # shares no project, volume, bridge, capture directory, or loopback
        # port with any prior attempt.
        @{
            HttpPort = '11802'; CompatPort = '11803'
            AppSubnet = '10.255.34.0/24'; GatewaySubnet = '10.186.0.0/16'
            BffGatewayIp = '10.186.0.10'
        }
    }
    'attempt40' {
        # attempt39 remains preserved with a valid direct proof bound to the
        # prior source closure. attempt40 is a newly allocated, empty direct
        # laboratory for the current source closure; it shares no project,
        # volume, bridge, capture directory, or loopback port with any prior
        # attempt.
        @{
            HttpPort = '11804'; CompatPort = '11805'
            AppSubnet = '10.255.35.0/24'; GatewaySubnet = '10.184.0.0/16'
            BffGatewayIp = '10.184.0.10'
        }
    }
}
