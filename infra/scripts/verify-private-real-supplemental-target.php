...
try {
    ...
    ob_end_clean();
    if (!class_exists(Schema::class) || Schema::CURRENT_VERSION !== 16) {
        throw new RuntimeException('schema_source_not_v16');
    }
    ...
    if ($migration === null || (int) ($migration['schema_version'] ?? 0) !== 16
        || (int) ($migration['ledger_count'] ?? 0) !== 16
    ) {
        throw new RuntimeException('target_schema_not_exact_v16');
    }
    ...
    if ($action === 'schema') {
        fwrite(STDOUT, "PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=schema schema=16 source_paths=NOT_READ\n");
        exit(0);
    }
    ...
    fwrite(STDOUT, 'PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=postflight schema=16 sources=28 presentations=26 '
