...
# Portable v2 stream restore helper. PowerShell validates the NTFS bundle,
# portable envelope and owner-only passphrase files before crossing into WSL.
# This helper talks only to exact v2-labelled containers and M-backed volumes;
# decrypted databases and tar members are streamed directly to those targets.

umask 077
export LC_ALL=C

fail() {
  printf '%s\n' "OWNER_RESTORE_V2_STREAM=FAIL code=$1" >&2
  exit 1
}
...
source_root= bundle= passphrase_file= piwigo_env=
while [ "$#" -gt 0 ]; do
  case "$1" in
    --source-root) [ "$#" -ge 2 ] || fail argument_missing; source_root=$2; shift 2 ;;
...
case "$source_root" in */ClassArchive-Independent-Recovery) ;; *) fail source_root_invalid ;; esac
case "$bundle" in "$source_root"/bundles/owner-full-v2-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]T[0-9][0-9][0-9][0-9][0-9][0-9]Z) ;; *) fail bundle_path_invalid ;; esac
case "$passphrase_file" in */.codex-work/owner-restore-v2/runtime/owner-full-v2-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]T[0-9][0-9][0-9][0-9][0-9][0-9]Z/archive-passphrase.txt) ;; *) fail passphrase_path_invalid ;; esac
...
[ "$passphrase_bytes" -ge 32 ] && [ "$passphrase_bytes" -le 512 ] || fail passphrase_shape_invalid
...
gpg_home=$(mktemp -d /tmp/class-archive-owner-restore-v2-gpg.XXXXXXXX) || fail gpg_temp_failed
chmod 0700 "$gpg_home"
...
piwigo_project=class_archive_owner_restore_v2_piwigo
immich_project=class_archive_owner_restore_v2_immich
mariadb_container=${piwigo_project}-db-1
postgres_container=${immich_project}-database-1
piwigo_data=class_archive_owner_restore_v2_piwigo_data
piwigo_scripts=class_archive_owner_restore_v2_piwigo_scripts
piwigo_uploads=class_archive_owner_restore_v2_piwigo_uploads
piwigo_galleries=class_archive_owner_restore_v2_piwigo_galleries
immich_upload=class_archive_owner_restore_v2_immich_upload
...
assert_container() {
  ...
  [ "$identity" = "$project|$service|owner-independent-restore-v2|true" ] || fail restore_container_identity_invalid
}

assert_volume() {
  expected=$1 project=$2 logical=$3
  expected_device=/mnt/classarchive-owner-restore-v2/volumes/$expected
  ...
  [ "$identity" = "local|none|bind|$expected_device|$project|$logical|owner-independent-restore-v2|m-ext4-bind-v2" ] || fail restore_volume_identity_invalid
  [ -d "$expected_device" ] && [ ! -L "$expected_device" ] || fail restore_volume_backing_untrusted
  [ "$(findmnt -n -o TARGET -T "$expected_device")" = /mnt/classarchive-owner-restore-v2 ] || fail restore_volume_mount_invalid
  ...
  [ "$(blkid -s LABEL -o value "$volume_device")" = CLASSARCHIVE_V2 ] || fail restore_volume_mount_invalid
}
...
case "$action" in
  verify)
    ...
    decrypt "$bundle/databases/immich-postgres.dump.gpg" | \
      "${docker_host[@]}" exec -i --user postgres "$postgres_container" sh -eu -c '
        dump=$(mktemp /tmp/class-archive-owner-v2-verify.XXXXXXXX)
  ...
  restore-immich-postgres)
    ...
    decrypt "$bundle/databases/immich-postgres.dump.gpg" | \
      "${docker_host[@]}" exec -i --user postgres "$postgres_container" sh -eu -c '
        dump=$(mktemp /tmp/class-archive-owner-v2-restore.XXXXXXXX)
        trap '\''rm -f -- "$dump"'\'' EXIT HUP INT TERM
        cat > "$dump"
  ...
  write-piwigo-config)
    case "$piwigo_env" in */infra/owner-restore-v2/.env.piwigo) ;; *) fail piwigo_env_path_invalid ;; esac
...
printf '%s\n' "OWNER_RESTORE_V2_STREAM=PASS action=$action"
