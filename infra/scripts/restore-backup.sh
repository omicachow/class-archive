...
# A bundle must select one exact known contract before any target is cleared.
# Format 8/schema 16 remains the historical path; format 9/schema 17 adds only
# the persisted Collections-first business domain. Unknown or mismatched
# format/schema claims fail closed at this pre-target boundary.
ca_recovery_select_manifest "$bundle/MANIFEST.json" || fail restore_business_manifest_invalid
expected_schema="$CA_RECOVERY_SCHEMA_VERSION"
expected_format="$CA_RECOVERY_FORMAT"
...
# Validate every destructive target and the empty database precondition before
# clearing even one media volume. A wrong/non-empty database must not turn a
# failed restore invocation into a partial media erase.
...
# Re-check the selected manifest contract against the restored database before
# any application volume is repopulated. Identifiers are accepted only from the
...
[ "$ci_version" = "$expected_schema" ] || fail restore_class_identity_schema_invalid
...
printf '%s\n' "BACKUP_RESTORE=PASS bundle=$CLASS_ARCHIVE_RESTORE_BUNDLE format=$expected_format schema=$expected_schema"
