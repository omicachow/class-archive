...
try {
    ...
    if ($Action -eq 'validate') {
        Write-Output ('PRIVATE_REAL_SUPPLEMENTAL_APPLY=PASS action=validate target=' + $Target `
            + ' schema=REQUIRED_V17 sources=28 presentations=26 expected_applied=26 expected_deduplicated=2 ' `
    ...
    Assert-Apply (@($schemaLines | Where-Object { $_ -eq 'PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=schema schema=17 source_paths=NOT_READ' }).Count -eq 1) 'schema_preflight_result_invalid'
    ...
    $preflight = @($lines | Where-Object { $_ -match '^PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=preflight schema=17 sources=28 presentations=26 mode=(?:FRESH|RESUME|REPLAY) source_existing=[0-9]+ canonical_existing=[0-9]+ source_paths=NOT_PRESENT$' })
    ...
    $postflight = @($lines | Where-Object { $_ -eq 'PRIVATE_REAL_SUPPLEMENTAL_TARGET=PASS action=postflight schema=17 sources=28 presentations=26 applied=26 deduplicated=2 failed=0 idempotent=PASS source_paths=NOT_PRESENT' })
    ...
    Write-Output ('PRIVATE_REAL_SUPPLEMENTAL_APPLY=PASS action=apply target=' + $Target `
        + ' schema=17 sources=28 presentations=26 durable_applied=26 durable_deduplicated=2 failed=0 ' `
