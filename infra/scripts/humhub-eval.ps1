...
$composeArguments = @(
    ...
    'docker', 'compose',
    '--env-file', '.env',
    '-f', 'infra/docker-compose.humhub-eval.yml'
)
...
switch ($Action) {
    ...
    'modules' {
        $commandArguments = $composeArguments + @(
            'exec', '-T', '--user', 'www-data', 'humhub',
            'php', '/workspace/infra/scripts/install-locked-humhub-modules.php'
        )
    }
    'modules-verify' {
        $commandArguments = $composeArguments + @(
            'exec', '-T', '--user', 'www-data', 'humhub',
            'php', '/workspace/infra/scripts/install-locked-humhub-modules.php', '--verify-only'
        )
    }
