...
switch ($Action) {
    ...
    'test-phase3-contract' {
        ...
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/collection-snapshot-builder-runtime.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/spotlight-rotation-schema-semantics.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
        ...
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/collection-snapshot-builder-static.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/collection-snapshot-fallback-contract.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/collection-lifecycle-maintenance.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/collection-feedback-projection-contract.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/memory-album-curation-contract.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
        ...
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & node.exe (Join-Path $projectRoot 'tests\phase3\photos-app-v4-contract.mjs')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & node.exe (Join-Path $projectRoot 'tests\phase3\photos-app-v4-ui-contract.mjs')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $projectRoot 'tests\phase3\photos-app-v4-chrome-qa-protocol.ps1')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $projectRoot 'tests\phase3\photos-app-v4-chrome-deep-qa-protocol.ps1')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $projectRoot 'tests\phase3\photos-app-v4-chrome-scope-projection-protocol.ps1')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
        ...
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/spotlight-rotation-migration-contract.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & wsl.exe @($composeArguments + @(
            'exec', '-T', '--user', 'nginx', 'piwigo',
            'php', '/workspace/tests/phase3/spotlight-rotation-service.php'
        ))
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
        ...
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $projectRoot 'tests\phase3\private-v18-owner-migration-protocol.ps1')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
            (Join-Path $projectRoot 'tests\phase3\v4-synthetic-phase-ab-attestation-protocol.ps1')
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
