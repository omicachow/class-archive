...
hard-coded laboratory identity: attempt40 on loopback ports 11804/11805.  The
...
unexpected child exit; attempt26 is preserved after its ambiguous exit object;
attempt27 is preserved after its custom cold-start wait; attempt28 is
preserved after its temporary HTTP health wait; attempt29 is preserved after
an unbounded Compose control call exhausted the outer deadline; attempt30 is
preserved after an explicit initialise prerequisite was not run; attempt31 is
preserved after a bounded probe minimum mismatch; attempt32 is preserved with
head-stale evidence; attempt33 is preserved after the Owner migration
Compose-argument binding repair; attempt35 is preserved after the V16
snapshot-recovery correction; attempt36 remains preserved with its prior
head-bound proof; attempt38 is preserved with evidence bound to the former
Docker-control-plane maintenance probe. attempt39 is the fresh bounded-control
replacement laboratory for the previous source closure. attempt40 is the
fresh current-source replacement laboratory.
#>
[CmdletBinding()]
param(
...
$httpPort = '11804'
$compatPort = '11805'
$composeProject = 'class_archive_v18_synthetic_migration_attempt40'
...
function Invoke-NativeCapture([string]$FileName, [string[]]$Arguments, [string]$FailureCode, [string]$ChildFailureStopPrefix = '', [int]$TimeoutSeconds = 240) {
    ...
    if ($TimeoutSeconds -lt 1 -or $TimeoutSeconds -gt 600) { Stop-V16ToV18DirectRuntime 'native_timeout_invalid' }
...
function Invoke-BaseRunner([string]$BaseAction, [switch]$RestoreConfirmation) {
    ...
    if ($baseRunnerTimeoutSeconds -lt 300 -or $baseRunnerTimeoutSeconds -gt 900) { Stop-V16ToV18DirectRuntime 'base_runner_timeout_invalid' }
    ...
    if (-not $process.WaitForExit($baseRunnerTimeoutSeconds * 1000)) {
    ...
    if (-not $process.HasExited) {
        $process.Dispose()
        Stop-V16ToV18DirectRuntime 'base_runner_exit_state_invalid'
    }
...
function Invoke-DirectCompose([string[]]$ComposeArguments) {
    ...
    return @(Invoke-NativeCapture $wsl $all ('direct_compose_failed_' + $stageCode))
}

function Invoke-DirectDocker([string[]]$DockerArguments, [string]$FailureCode, [int]$TimeoutSeconds = 30) {
    Assert-DockerDesktopEnginePipe
    if ($FailureCode -notmatch '^[a-z0-9_]{3,96}$') { Stop-V16ToV18DirectRuntime 'direct_docker_failure_code_invalid' }
    return @(Invoke-NativeCapture $wsl (@('-d','Ubuntu','--exec','docker') + $DockerArguments) $FailureCode '' $TimeoutSeconds)
}

function Get-DirectContainerName([string]$Service) {
    if ($Service -notin @('db','piwigo')) { Stop-V16ToV18DirectRuntime 'direct_service_name_invalid' }
    return ($composeProject + '-' + $Service + '-1')
}

function Assert-DirectContainer([string]$Service) {
    $container = Get-DirectContainerName $Service
    # Docker's narrow name projection is used instead of Compose ps so the
    # direct proof does not rely on an unbounded Compose control call. The
    # fixed project label and exact expected name must both match before an
    # exec command can address the synthetic laboratory.
    $lines = @(Invoke-DirectDocker @('ps','-a','--filter',('label=com.docker.compose.project=' + $composeProject),'--filter',('name=' + $container),'--format={{.Names}}') ('direct_container_lookup_' + $Service) 15)
    if ($lines.Count -ne 1 -or $lines[0] -ne $container) { Stop-V16ToV18DirectRuntime ('direct_container_identity_invalid_' + $Service) }
    return $container
}

function Get-DirectSchemaVersion {
    $db = Assert-DirectContainer 'db'
    $lines = @(Invoke-DirectDocker @('exec',$db,'sh','/workspace/infra/scripts/v18-synthetic-db-probe.sh','schema') 'direct_schema_probe' 30)
...
function Invoke-DirectProof([string]$Mode) {
    ...
    $piwigo = Assert-DirectContainer 'piwigo'
    $proofCode = ('direct_proof_' + ($script:stage -replace '[^a-z0-9_]', '_'))
    $lines = @(Invoke-DirectDocker @(
        'exec','--user','nginx',
...
function Invoke-Initialize {
    ...
    $record = @($lines | Where-Object { $_ -eq 'V18_SYNTHETIC_MIGRATION=READY stage=initialize attempt=attempt40 source=V16_DB_ONLY historical_schema=V17_PINNED media=NOT_MOUNTED' })
    if ($record.Count -ne 1) { Stop-V16ToV18DirectRuntime 'base_initialize_evidence_invalid' }
    Get-SandboxValues | Out-Null
    Write-V16ToV18DirectRuntime 'PASS' 'initialize' 'attempt=attempt40 ports=127.0.0.1:11804_11805 source=V16_DB_ONLY media=NOT_MOUNTED'
}
...
function Invoke-Restore {
    ...
    $record = @($lines | Where-Object { $_ -match '^V18_SYNTHETIC_MIGRATION=PASS stage=restore schema=16 source=pre-migration-db-v16-to-v17-[0-9]{8}T[0-9]{6}Z target=attempt40 media=NOT_MOUNTED$' })
    ...
    Write-V16ToV18DirectRuntime 'PASS' 'restore' 'attempt=attempt40 schema=16 target=ISOLATED media=NOT_MOUNTED'
}
...
function Invoke-Prove {
    ...
    Write-V16ToV18DirectRuntime 'PASS' 'prove' ('attempt=attempt40 schema_from=16 schema_to=18 direct_current_source=PASS replay=PASS verify=PASS fail_closed=PASS legacy_fingerprint=' + $fingerprint + ' media=NOT_MOUNTED')
}
...
function Invoke-Verify {
    ...
    Write-V16ToV18DirectRuntime 'PASS' 'verify' ('attempt=attempt40 schema=18 direct_current_source=REPLAY_VERIFIED verify=PASS fail_closed=PASS legacy_fingerprint=' + $fingerprint + ' media=NOT_MOUNTED')
}
...
try {
    switch ($Action) {
        ...
        'status' {
            ...
            Write-V16ToV18DirectRuntime 'STATUS' 'status' ('attempt=attempt40 ports=127.0.0.1:11804_11805 initialized=' + $initialized.ToString().ToUpperInvariant() + ' proof=' + $proof.ToString().ToUpperInvariant() + ' media=NOT_MOUNTED')
        }
    }
} catch {
