...
function fixtureRecoverySchemaVersion(mysqli $db, string $ci): int
{
    $migration = $ci . 'migration';
    if (preg_match('/\A[A-Za-z0-9_]+\z/D', $migration) !== 1) {
        fixtureFail('migration_table_invalid');
    }
    $result = $db->query('SELECT COUNT(*) AS `count`,COALESCE(MAX(`version`),0) AS `version` FROM `' . $migration . '`');
    if (!$result instanceof mysqli_result || ($row = $result->fetch_assoc()) === null) {
        fixtureFail('migration_version_query_failed');
    }
    $result->free();
...
$schemaVersion = fixtureRecoverySchemaVersion($mysqli, $ci);
$tables = [
...
if ($schemaVersion === 17) {
    ...
    $tables += [
        'collection_snapshot' => 'SELECT HEX(`snapshot_id`) AS `snapshot_id`,`scope`,`projection_kind`,`state`,HEX(`input_revision`) AS `input_revision`,HEX(`payload_digest`) AS `payload_digest`,`item_count`,`created_at`,`published_at`,`superseded_at`,`updated_at` FROM `' . $ci . 'collection_snapshot` ORDER BY `scope`,`projection_kind`,`snapshot_id` ASC',
        'collection_snapshot_item' => 'SELECT HEX(`snapshot_id`) AS `snapshot_id`,`ordinal`,`item_kind`,SHA2(CAST(`item_key` AS CHAR),256) AS `item_key_sha256`,HEX(`cover_class_photo_id`) AS `cover_class_photo_id`,SHA2(CAST(`photo_ids_json` AS CHAR),256) AS `photo_ids_json_sha256`,SHA2(CAST(`payload_json` AS CHAR),256) AS `payload_json_sha256`,SHA2(COALESCE(JSON_UNQUOTE(JSON_EXTRACT(`payload_json`,\'$.title\')),\'\') ,256) AS `payload_title_sha256`,HEX(`payload_digest`) AS `payload_digest`,`created_at` FROM `' . $ci . 'collection_snapshot_item` ORDER BY `snapshot_id`,`ordinal` ASC',
        'collection_snapshot_pointer' => 'SELECT `scope`,`projection_kind`,HEX(`active_snapshot_id`) AS `active_snapshot_id`,HEX(`active_revision`) AS `active_revision`,`activated_at`,`updated_at` FROM `' . $ci . 'collection_snapshot_pointer` ORDER BY `scope`,`projection_kind` ASC',
...
if ($schemaVersion === 16) {
    // Preserve format-8 fixture bytes/shape for historical synthetic recovery
    // comparison. V17 data never changes this legacy contract.
    $payload = [
        'fixture_version' => 8,
