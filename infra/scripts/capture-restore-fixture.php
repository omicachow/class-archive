...
$tables = [
    ...
    'identity' => 'SELECT `id`,`roster_code`,`identity_type`,`state`,`auth_epoch` FROM `' . $ci . 'identity` ORDER BY `id` ASC',
    'seat' => 'SELECT `id`,`identity_id`,`seat_type`,`ordinal`,`state`,`invite_generation`,`lock_version` FROM `' . $ci . 'seat` ORDER BY `id` ASC',
    'account' => 'SELECT `id`,`seat_id`,`state`,`current_marker`,`auth_epoch` FROM `' . $ci . 'account` ORDER BY `id` ASC',
    'principal' => 'SELECT `id`,`account_id`,`system_role`,`principal_type`,`state`,`auth_epoch` FROM `' . $ci . 'principal` ORDER BY `id` ASC',
    'token_state' => 'SELECT `id`,`seat_id`,`purpose`,`generation`,`state`,`expires_at`,`used_at`,`revoked_at` FROM `' . $ci . 'token` ORDER BY `id` ASC',
