...
source closure exactly match the proof that ran in attempt26.  The emitted
...
function Get-RuntimeLockMetadata {
    ...
    if ($safe.COMPOSE_PROJECT_NAME -ne 'class_archive_v18_synthetic_migration_attempt26' -or $safe.CLASS_ARCHIVE_HTTP_PORT -ne $httpPort -or
...
function Create-Attestation {
    ...
    Write-V16ToV18DirectAttestation 'PASS' ('action=create commit=' + $material.head + ' source_digest=' + $material.source_digest + ' proof_sha256=' + $material.direct_proof_sha256 + ' attempt=attempt26 media=NOT_MOUNTED')
}
...
function Verify-Attestation {
    ...
    Write-V16ToV18DirectAttestation 'PASS' ('action=verify commit=' + $material.head + ' source_digest=' + $material.source_digest + ' proof_sha256=' + $material.direct_proof_sha256 + ' attempt=attempt26 media=NOT_MOUNTED')
}
...
try {
    switch ($Action) {
        ...
        'status' {
            ...
            Write-V16ToV18DirectAttestation 'STATUS' ('attempt=attempt26 attestation=' + $exists.ToString().ToUpperInvariant() + ' ports=127.0.0.1:11090_11091 media=NOT_MOUNTED')
        }
    }
} catch {
