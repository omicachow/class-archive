...
function New-RestoreNginxConfiguration {
    ...
    # The map evaluates $realip_remote_addr (the actual peer before the BFF's
    # X-Forwarded-For is applied), so restore-only BFFs must be added to both
    # the exact source map and the real-ip trust list.  Do not replace either
    # entry with a gateway CIDR: an unrelated container on the restore network
    # must not be able to mint the internal member-upload marker.
    $mapAnchor = '        10.241.0.10 1;'
    $realIpAnchor = '        set_real_ip_from 10.241.0.10/32;'
    Assert-Restore (([regex]::Matches($source, [regex]::Escape($mapAnchor))).Count -eq 1 -and
        ([regex]::Matches($source, [regex]::Escape($realIpAnchor))).Count -eq 1 -and
        -not $source.Contains('        10.245.0.10 1;') -and
        -not $source.Contains('set_real_ip_from 10.245.0.10/32;')) 'restore_nginx_trust_anchor_invalid'
    $generated = $source.Replace(
        $mapAnchor,
        $mapAnchor + "`n        # Restore-only compatibility BFF on the isolated gateway.`n        10.245.0.10 1;"
    ).Replace(
        $realIpAnchor,
        $realIpAnchor + "`n        # Restore-only compatibility BFF on the isolated gateway.`n        set_real_ip_from 10.245.0.10/32;"
    )
    Assert-Restore (([regex]::Matches($generated, '(^|\n)\s*10\.245\.0\.10 1;')).Count -eq 1 -and
        ([regex]::Matches($generated, 'set_real_ip_from 10\.245\.0\.10/32;')).Count -eq 1) 'restore_nginx_generation_failed'
