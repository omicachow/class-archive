...
final class GatewayHttpController
{
    ...
    public static function onBeginIndex(): void
    {
        ...
        try {
            ...
            if ($code === 'class_archive_gateway_principal_unresolved'
                ...
                || str_contains($code, '_memory_album_role_forbidden')
                || str_contains($code, '_album_cover_role_forbidden')
            ) {
                self::respond(403, ['error' => '禁止访问']);
            }
            if ($code === 'class_archive_gateway_photo_not_found'
                || $code === 'class_archive_gateway_album_not_found'
                || $code === 'class_archive_collection_snapshot_item_not_visible'
            ...
            if (str_contains($code, '_already_') || str_contains($code, '_drift') || str_contains($code, '_race')
                ...
                || str_contains($code, '_memory_album_mixed_era')
                || str_contains($code, '_private_arrangement_customization_forbidden')
    ...
    private static function gateway(): GatewayService
    {
        return new GatewayService(
            ...
            \ClassIdentity\CollectionSnapshotService::fromPiwigo(),
        );
    }
    ...
    private static function handleProductRead(array $segments, GatewayService $gateway): ?array
    {
        ...
        if ($segments === ['product-state']) {
            ...
            return [
                ...
                // A normal member publication is a separate explicit-era
                // workflow. Never infer this client hint from the seat shape:
                // only active Classmate/Teacher principals receive it.
                'canEraUpload' => in_array($role, [\ClassIdentity\Access::ROLE_CLASSMATE, \ClassIdentity\Access::ROLE_TEACHER], true),
                // Family keeps the pre-existing pending-review flow. This is
                // intentionally distinct from `canEraUpload` so a UI cannot
                // turn an invite seat into direct publication.
                'canFamilySubmission' => $role === \ClassIdentity\Access::ROLE_FAMILY,
        ...
        if ($segments === ['member-upload', 'options']) {
            self::requireExactQuery([]);
            return $gateway->memberEraUploadOptions();
        }
        if ($segments === ['home']) {
            self::requireExactQuery([]);
            return $gateway->home();
        }
        if ($segments === ['collections', 'home']) {
            self::requireExactQuery([]);
            return $gateway->collectionsHome();
        }
        if ($segments === ['collections', 'state']) {
            self::requireExactQuery([]);
            return $gateway->collectionsState();
        }
        if ($segments === ['collections', 'pins']) {
            self::requireExactQuery([]);
            return $gateway->collectionPins();
        }
        ...
        if ($segments === ['search', 'grouped']) {
            $params = self::requireExactQuery(
                ['q', 'contextType', 'contextId', 'albumId', 'cursor', 'limit'],
                ['contextType', 'contextId', 'albumId', 'cursor', 'limit'],
            );
            $query = $params['q'] ?? null;
            if (!is_string($query)) {
                throw new \InvalidArgumentException('class_archive_gateway_search_missing');
            }
            $hasContextType = isset($params['contextType']);
            $hasContextId = isset($params['contextId']);
            if ((!$hasContextType && $hasContextId)
                || ($hasContextType && (string) $params['contextType'] === SearchContext::ALL && $hasContextId)
                || ($hasContextType && (string) $params['contextType'] !== SearchContext::ALL && !$hasContextId)
            ) {
                throw new \InvalidArgumentException('class_archive_gateway_search_context_invalid');
            }
            $albumId = isset($params['albumId']) ? self::uuid($params['albumId']) : null;
            if ($albumId !== null && $hasContextType) {
                // Preserve one legacy compatibility form, but never permit
                // two potentially conflicting server scope instructions.
                throw new \InvalidArgumentException('class_archive_gateway_search_context_invalid');
            }
            $context = $albumId !== null
                ? SearchContext::legacyAlbum($albumId)
                : SearchContext::fromRequest(
                    $hasContextType ? (string) $params['contextType'] : SearchContext::ALL,
                    $hasContextId ? (string) $params['contextId'] : null,
                );
            ...
            if (isset($params['limit'])) {
                if (preg_match('/\A[1-9][0-9]{0,2}\z/D', $params['limit']) !== 1) {
            ...
            return $gateway->groupedSearch($query, $context, $cursor, $limit);
        }
    ...
    private static function handleMutation(array $segments, GatewayService $gateway): array
    {
        ...
        $contracts = [
            ...
            'collections/pins/create' => [
                ['csrfToken', 'projectionKind', 'itemKind', 'itemKey'],
                ['csrfToken', 'projectionKind', 'itemKind', 'itemKey'],
            ],
            ...
            'collections/feedback/set' => [
                ['csrfToken', 'projectionKind', 'itemKind', 'itemKey', 'feedback'],
                ['csrfToken', 'projectionKind', 'itemKind', 'itemKey', 'feedback'],
            ],
            ...
            'collections/memories/save-as-album' => [
                ['csrfToken', 'memoryKey', 'title', 'description', 'reason'],
                ['csrfToken', 'memoryKey', 'reason'],
            ],
        ...
        return match ($route) {
            ...
            'collections/pins/create' => self::mutateCollectionPinCreate($gateway, $body),
            'collections/pins/remove' => self::mutateCollectionPinRemove($gateway, $body),
            'collections/pins/reorder' => self::mutateCollectionPinReorder($gateway, $body),
            'collections/feedback/set' => self::mutateCollectionFeedbackSet($gateway, $body),
            'collections/feedback/clear' => self::mutateCollectionFeedbackClear($gateway, $body),
            'collections/memories/save-as-album' => self::mutateMemorySaveAsAlbum($gateway, $body),
            'collections/albums/cover' => self::mutateCollectionAlbumCover($gateway, $body),
            default => throw new \RuntimeException('class_archive_gateway_route_not_found'),
        };
    }

    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionPinCreate(GatewayService $gateway, array $body): array
    {
        return $gateway->pinCollection(
            self::collectionProjectionKind($body['projectionKind'] ?? null),
            self::collectionItemKind($body['itemKind'] ?? null),
            self::collectionItemKey($body['itemKey'] ?? null),
        );
    }

    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionPinRemove(GatewayService $gateway, array $body): array
    {
        return $gateway->unpinCollection(
            self::collectionProjectionKind($body['projectionKind'] ?? null),
            self::collectionItemKind($body['itemKind'] ?? null),
            self::collectionItemKey($body['itemKey'] ?? null),
        );
    ...
    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionPinReorder(GatewayService $gateway, array $body): array
    {
        $pins = $body['pins'] ?? null;
        if (!is_array($pins) || count($pins) < 1 || count($pins) > 100) {
        ...
        foreach ($pins as $pin) {
            ...
            if ($keys !== ['itemKey', 'itemKind', 'projectionKind']) {
                throw new \InvalidArgumentException('class_archive_collection_pin_reorder_invalid');
            }
            $normalized[] = [
                'projectionKind' => self::collectionProjectionKind($pin['projectionKind'] ?? null),
                'itemKind' => self::collectionItemKind($pin['itemKind'] ?? null),
                'itemKey' => self::collectionItemKey($pin['itemKey'] ?? null),
            ];
        }
        return $gateway->reorderCollectionPins($normalized);
    }

    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionFeedbackSet(GatewayService $gateway, array $body): array
    {
        return $gateway->setCollectionFeedback(
            self::collectionProjectionKind($body['projectionKind'] ?? null),
            self::collectionItemKind($body['itemKind'] ?? null),
            self::collectionItemKey($body['itemKey'] ?? null),
            self::collectionFeedbackKind($body['feedback'] ?? null),
        );
    }

    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionFeedbackClear(GatewayService $gateway, array $body): array
    {
        return $gateway->clearCollectionFeedback(
            self::collectionProjectionKind($body['projectionKind'] ?? null),
            self::collectionItemKind($body['itemKind'] ?? null),
            self::collectionItemKey($body['itemKey'] ?? null),
        );
    ...
    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateMemorySaveAsAlbum(GatewayService $gateway, array $body): array
    {
        return $gateway->saveMemoryAsAlbum(
            self::collectionItemKey($body['memoryKey'] ?? null),
            self::nullableText($body['title'] ?? null, 190),
            self::nullableText($body['description'] ?? null, 4000),
            self::reason($body),
        );
    }

    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateCollectionAlbumCover(GatewayService $gateway, array $body): array
    {
        return $gateway->setAlbumCover(
            self::uuid($body['albumId'] ?? null),
            self::uuid($body['photoId'] ?? null),
            self::reason($body),
        );
    }
    ...
    /** @param array<string,mixed> $body @return array<string,mixed> */
    private static function mutateAlbumCover(GatewayService $gateway, array $body): array
    {
        $albumId = self::uuid($body['albumId'] ?? null);
        $photoId = self::uuid($body['photoId'] ?? null);
        // Keep the historical Admin endpoint, but route it through the same
        // actor-checked direct-membership + effective-era curation path as
        // the member-facing Collections contract. A paged browser album is
        // never used to decide whether a photo may become a cover.
        return $gateway->setAlbumCover($albumId, $photoId, self::reason($body));
    }
    ...
    private static function collectionProjectionKind(mixed $value): string
    {
        if (!is_string($value)) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_projection_kind_invalid');
        }
        $value = strtoupper(trim($value));
        if (!in_array($value, ['HOME', 'MEMORY', 'SPOTLIGHT', 'SEARCH_SUGGESTION'], true)) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_projection_kind_invalid');
        }
        return $value;
    }

    private static function collectionItemKind(mixed $value): string
    {
        if (!is_string($value)) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_item_kind_invalid');
        }
        $value = strtoupper(trim($value));
        if (!in_array($value, ['AUTO_COLLECTION', 'ALBUM', 'PERSON', 'SPOTLIGHT', 'PHOTO', 'SEARCH_SUGGESTION'], true)) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_item_kind_invalid');
        }
        return $value;
    }

    private static function collectionItemKey(mixed $value): string
    {
        if (!is_string($value)) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_item_key_invalid');
        }
        $value = trim($value);
        if (preg_match('/\A[A-Za-z0-9][A-Za-z0-9:_-]{0,95}\z/D', $value) !== 1) {
            throw new \InvalidArgumentException('class_archive_collection_snapshot_item_key_invalid');
        }
        return $value;
    }

    private static function collectionFeedbackKind(mixed $value): string
    {
        if (!is_string($value)) {
            throw new \InvalidArgumentException('class_archive_collection_feedback_kind_invalid');
        }
        $value = strtoupper(trim($value));
        if (!in_array($value, ['HIDE', 'LESS_LIKE', 'LIKE'], true)) {
            throw new \InvalidArgumentException('class_archive_collection_feedback_kind_invalid');
        }
        return $value;
    }
