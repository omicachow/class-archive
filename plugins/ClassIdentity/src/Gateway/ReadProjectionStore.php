...
final class ReadProjectionStore
{
     ...
     * only the selected rows, or atomically appends new canonical rows to the
     * current generation. It recomputes the authorization-neutral catalog
     * digest from stored row digests and republishes the meta row last. An
     * append remains deliberately bounded: it is not a source scan, does not
     * rotate the catalog generation, and rejects a Piwigo or canonical-id
     * collision rather than repairing it opportunistically.
     *
     * @param list<GatewayPhotoCandidate> $photos
     * @param list<string> $affectedAggregateKinds
     * @param array{catalog_generation:string,native_source_epoch:string,aggregate_epochs:array<string,string>} $buildToken
     * @return array{changed:bool,updated:int,appended:int,count:int,source_revision:string,generation:string}
     */
    public function refreshPhotos(array $photos, array $affectedAggregateKinds, array $buildToken = []): array
    {
        ...
        $result = $this->repository->transaction(function () use ($encoded, $normalizedKinds, $buildToken): array {
            ...
            if ($catalog === null
                ...
                // A full rebuild may legitimately publish an empty archive.
                // Keep that active zero-row generation appendable for the
                // first bounded member upload; a negative/missing count is
                // still an invalid catalog state.
                || (int) ($catalog['item_count'] ?? -1) < 0
            ) {
            ...
            foreach ($existing as $row) {
                ...
                $id = (string) $row['class_photo_id'];
                $imageId = (int) ($row['piwigo_image_id'] ?? 0);
                if ($imageId <= 0 || isset($existingById[$id])) {
                    throw new \RuntimeException('class_archive_read_projection_refresh_row_invalid');
                }
                $existingById[$id] = $imageId;
            }
            $imageIds = array_column($encoded, 'image_id');
            $existingImages = $this->repository->fetchAll(
                "SELECT `class_photo_id`,`piwigo_image_id` FROM {$rows} WHERE `generation`=? AND `piwigo_image_id` IN ("
                    . implode(',', array_fill(0, count($imageIds), '?')) . ') FOR UPDATE',
                array_merge([$generation], $imageIds),
            );
            $existingByImage = [];
            foreach ($existingImages as $row) {
                ...
                $id = (string) $row['class_photo_id'];
                $imageId = (int) ($row['piwigo_image_id'] ?? 0);
                if ($imageId <= 0 || isset($existingByImage[$imageId])) {
                    throw new \RuntimeException('class_archive_read_projection_refresh_row_invalid');
                }
                $existingByImage[$imageId] = $id;
            }
            $changed = false;
            $appended = 0;
            foreach ($encoded as $row) {
                $existingImageId = $existingById[$row['id']] ?? null;
                $existingClassIdForImage = $existingByImage[$row['image_id']] ?? null;
                if ($existingImageId === null) {
                    if ($existingClassIdForImage !== null) {
                    ...
                    if ($inserted !== 1) {
                        throw new \RuntimeException('class_archive_read_projection_refresh_append_failed');
                    }
                    $changed = true;
                    ++$appended;
                    continue;
                }
                if ($existingImageId !== $row['image_id'] || $existingClassIdForImage !== $row['id']) {
                    throw new \RuntimeException('class_archive_read_projection_refresh_identity_drift');
                }
                $current = $this->repository->fetchOne(
            ...
            if ($catalogCount !== (int) $catalog['item_count'] + $appended) {
            ...
            $published = $this->repository->execute(
                "UPDATE {$meta} SET `state`='ACTIVE',`source_revision`=?,`item_count`=?,`invalidated_reason`=NULL,"
                    . '`built_at`=UTC_TIMESTAMP(6),`invalidated_at`=NULL,`updated_at`=UTC_TIMESTAMP(6) '
                    . "WHERE `projection_key`=? AND `state`='STALE' AND `generation`=?",
                [$revision, $catalogCount, self::PHOTO_CATALOG, $generation],
            );
            if ($published !== 1) {
