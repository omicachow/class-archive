...
final class ReconciliationService
{
    ...
    public function scan(): array
    {
        ...
        // Direct member publication is a Core/MyISAM -> ClassIdentity/InnoDB
        // saga. It intentionally leaves an uncertain Core image at level 16
        // instead of deleting bytes or releasing visibility. Surface only the
        // three durable member-upload signatures below: a recorded abort, a
        // mapped non-submission archive image whose visibility release has not
        // completed, or an otherwise-unmapped level-16 upload with this
        // service's opaque 40-hex filename shape. The latter covers a crash
        // between Core creation and the point at which its new row can be
        // proven request-owned. This is always manual review; reconciliation
        // never makes a quarantined original public and never guesses a
        // compensating delete.
        $memberUploadQuarantines = $this->all(
            'SELECT DISTINCT i.`id` FROM `' . $this->prefix . 'images` i '
                . 'LEFT JOIN ' . $archiveTable . ' ai ON ai.`piwigo_image_id`=i.`id` '
                . 'LEFT JOIN ' . $photoTable . ' p ON p.`piwigo_image_id`=i.`id` '
                . 'LEFT JOIN `' . $this->prefix . 'class_identity_audit_event` ae '
                . "ON ae.`action`='MEMBER_ERA_UPLOAD_ABORT' AND ae.`target_type`='PHOTO_UPLOAD' "
                . 'AND ae.`target_id`=CAST(i.`id` AS CHAR) '
                . "WHERE i.`level`=16 AND i.`path` LIKE 'upload/%' AND (ae.`id` IS NOT NULL "
                . "OR (ai.`id` IS NOT NULL AND ai.`official`=0 AND ai.`source_submission_id` IS NULL "
                . 'AND p.`class_photo_id` IS NOT NULL) '
                . "OR (p.`class_photo_id` IS NULL AND i.`file` REGEXP '^[0-9a-f]{40}[.](jpg|png|webp)$')) ORDER BY i.`id` ASC",
        );
        foreach ($memberUploadQuarantines as $row) {
            $issues[] = self::issue('MEMBER_UPLOAD_QUARANTINED_REQUIRES_REVIEW', 'MANUAL_REVIEW', 'image:' . (int) ($row['id'] ?? 0));
        }
