...
final class ClassIdentitySubmissionService
{
    ...
    public function submit(int $userId, string $era, array $file, ?string $suggestedDate, string $precision, ?string $suggestedAlbum, ?string $description): int
    {
        // Do not let a future CLI/internal caller silently default into the
        // Family pending queue. This domain service has one fixed business
        // scope: a Family submission is explicitly HERITAGE-only.
        if ($era !== 'HERITAGE') {
