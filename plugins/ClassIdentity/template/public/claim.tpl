...
<header class="ca-public__head">
  <h2>认领班级身份</h2>
  <p class="ca-public__muted">此入口同时用于同学编号和老师编号。一次性认领码只能使用一次。</p>
</header>
...
{else}
  <form class="ca-public__panel ca-public__form" method="post" action="{$CA_CLAIM_URL|escape:'html'}" autocomplete="off">
    ...
    <label>同学编号 / 老师编号<input name="roster_code" type="text" maxlength="64" required autocapitalize="characters" autocomplete="off" placeholder="C25-018 或 T-001"></label>
    <label>一次性认领码<input name="claim_code" type="password" maxlength="128" required autocomplete="one-time-code" spellcheck="false"></label>
