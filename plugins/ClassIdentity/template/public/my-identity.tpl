    ...
    <div class="ca-public__panel">
      ...
      <ul class="ca-public__seats">
        {foreach from=$CA_MY.seats item=seat}
          <li class="ca-public__seat"><div><span class="ca-public__seat-main">Seat {$seat.ordinal|escape:'html'} · {$seat.type_label|escape:'html'}</span>{if $seat.account_name}<div>{$seat.account_name|escape:'html'}{if $seat.relationship_label} · {$seat.relationship_label|escape:'html'}{/if}</div>{/if}</div><div class="ca-public__seat-meta">{$seat.state_label|escape:'html'}{if $seat.account_state_label != '—'} · 账号 {$seat.account_state_label|escape:'html'}{/if}{if $seat.invite_expires_at}<br>邀请到期 {$seat.invite_expires_at|escape:'html'} UTC{/if}</div></li>
        {/foreach}
      </ul>
    </div>
    <div class="ca-public__panel">
      ...
      <div class="ca-public__actions">
        <form method="post" action="{$CA_MY_IDENTITY_URL|escape:'html'}"><input type="hidden" name="pwg_token" value="{$CA_PWG_TOKEN|escape:'html'}"><input type="hidden" name="action" value="issue_family_invitation"><button type="submit"{if !$CA_MY.can_issue_family} disabled{/if}>生成 Family Invitation</button></form>
