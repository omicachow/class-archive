...
<form method="post" class="ca-admin__form" autocomplete="off">
  ...
  <label>老师编号<input name="roster_code" maxlength="64" placeholder="T-001" required></label>
...
<table><thead><tr><th>席位</th><th>状态</th><th>账号</th><th>邮箱</th><th>注册时间</th></tr></thead><tbody>{foreach from=$CA_IDENTITY.seats item=seat}<tr><td>{$seat.seat_type_label|escape:'html'}</td><td>{$seat.state_label|escape:'html'}</td><td>{if $seat.username}{$seat.username|escape:'html'}{else}—{/if}</td><td>{if $seat.mail_address}{$seat.mail_address|escape:'html'}{else}—{/if}</td><td>{if $seat.bound_at}{$seat.bound_at|escape:'html'}{else}—{/if}</td></tr>{/foreach}</tbody></table>
<div class="ca-admin__actions"><form method="post" class="ca-admin__inline" autocomplete="off" data-confirm="签发新认领码会立即废止旧认领码，确认继续？"><input type="hidden" name="pwg_token" value="{$CA_PWG_TOKEN|escape:'html'}"><input type="hidden" name="action" value="reissue_claim"><input type="hidden" name="identity_id" value="{$CA_IDENTITY.id|escape:'html'}"><label>签发 / 重发原因<input name="reason" maxlength="500" required></label><button type="submit">生成一次性老师认领码</button></form>
