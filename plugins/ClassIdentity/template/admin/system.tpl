...
{if isset($CA_SYSTEM)}
{if $CA_SYSTEM.production_blocked}<div class="ca-admin__blocked">禁止投入生产（PRODUCTION BLOCKED）— 至少一项生产安全门禁失败。</div>{else}<div class="ca-admin__alert ca-admin__alert--success">当前可检测的安全底座与迁移检查已通过。</div>{/if}
<div class="ca-admin__panel"><h3>系统健康</h3><table><tbody>
<tr><th>媒体访问防护配置（MediaGuard）</th><td>{if $CA_SYSTEM.media_guard == 'CONFIGURED'}<span class="ca-admin__badge ca-admin__badge--ok">已配置（CONFIGURED）</span>{else}<span class="ca-admin__badge ca-admin__badge--danger">失败（FAIL）</span>{/if}</td></tr>
...
<tr><th>身份权限强制（Identity Enforcement）</th><td>{$CA_SYSTEM.identity_enforcement|escape:'html'}</td></tr><tr><th>独立系统管理员（SYSTEM_ADMIN）</th><td>{$CA_SYSTEM.system_admins|escape:'html'}</td></tr><tr><th>业务角色映射</th><td>{$CA_SYSTEM.role_group_mappings|escape:'html'}</td></tr><tr><th>密钥配置</th><td>{$CA_SYSTEM.secret_configuration|escape:'html'}</td></tr>
<tr><th>数据库</th><td>{$CA_SYSTEM.database|escape:'html'}</td></tr><tr><th>迁移</th><td>{$CA_SYSTEM.migration|escape:'html'}</td></tr><tr><th>数据结构校验（Schema）</th><td>{$CA_SYSTEM.schema_verification|escape:'html'}</td></tr><tr><th>缺失表</th><td>{if $CA_SYSTEM.missing_tables}{$CA_SYSTEM.missing_tables|escape:'html'}{else}无{/if}</td></tr>
...
<tr><th>最近成功备份</th><td>{$CA_SYSTEM.backup_last_success|escape:'html'}</td></tr><tr><th>最近失败备份</th><td>{$CA_SYSTEM.backup_last_failure|escape:'html'}</td></tr><tr><th>定时任务</th><td>{$CA_SYSTEM.cron_jobs|escape:'html'}</td></tr>
<tr><th>管理员多因素认证（MFA）</th><td>{$CA_SYSTEM.admin_mfa|escape:'html'}</td></tr><tr><th>生产阻断项</th><td>{$CA_SYSTEM.production_blockers|escape:'html'}</td></tr>
<tr><th>人工失败操作（FAILED_MANUAL）</th><td>{$CA_SYSTEM.failed_manual_operations|escape:'html'}</td></tr>
<tr><th>待补偿账号（COMPENSATION_REQUIRED）</th><td>{$CA_SYSTEM.compensation_required_accounts|escape:'html'}</td></tr>
<tr><th>账号开通健康度</th><td>{$CA_SYSTEM.provisioning_health|escape:'html'}</td></tr>
...
<table><thead><tr><th>操作</th><th>身份 / 席位</th><th>状态</th><th>错误码</th><th>更新时间</th><th>处理</th></tr></thead><tbody>
