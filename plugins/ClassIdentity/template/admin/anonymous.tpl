...
<table><thead><tr><th>对外匿名名</th><th>上下文</th><th>所属成员（管理员可见）</th><th>状态</th><th>互动摘要</th><th>操作</th></tr></thead><tbody>
{foreach from=$CA_ANONYMOUS item=row}
<tr><td>{$row.alias|escape:'html'}</td><td>{$row.context_label|escape:'html'}</td><td>{$row.roster_code|escape:'html'} · {$row.real_name|escape:'html'}<br>匿名席位</td><td>{$row.seat_state_label|escape:'html'} / {$row.account_state_label|escape:'html'}</td><td>{$row.comment_count|escape:'html'} 条评论{if $row.last_comment_at}<br>{$row.last_comment_at|escape:'html'}{/if}<br>举报：当前未接入</td><td>
