...
<form method="post" class="ca-admin__form" autocomplete="off">
  ...
  <label>Classmate ID<input name="roster_code" maxlength="64" placeholder="C25-001" required></label>
...
{foreach from=$CA_IDENTITY.seats item=seat}<tr><td>Seat {$seat.ordinal|escape:'html'}</td><td>{$seat.seat_type|escape:'html'}</td><td>{$seat.state|escape:'html'}</td><td>{if $seat.username}{$seat.username|escape:'html'}{else}—{/if}</td><td>{if $seat.account_real_name}{$seat.account_real_name|escape:'html'}{/if}{if $seat.family_relationship} · {$seat.family_relationship|escape:'html'}{/if}</td><td>{if $seat.bound_at}{$seat.bound_at|escape:'html'}{else}—{/if}</td></tr>{/foreach}
</tbody></table>
<div class="ca-admin__actions">
  <form method="post" class="ca-admin__inline" autocomplete="off" data-confirm="签发新 Code 会立即废止旧 Code，确认继续？"><input type="hidden" name="pwg_token" value="{$CA_PWG_TOKEN|escape:'html'}"><input type="hidden" name="action" value="reissue_claim"><input type="hidden" name="identity_id" value="{$CA_IDENTITY.id|escape:'html'}"><label>签发 / 重发原因<input name="reason" maxlength="500" required></label><button type="submit">生成一次性 Claim Code</button></form>
...
{/if}
<div class="ca-admin__panel"><h3>同学身份</h3><table><thead><tr><th>Classmate ID</th><th>真实姓名</th><th>本人席位</th><th>家庭席位</th><th>匿名席位</th><th>身份状态</th><th>注册时间</th><th>最近活动</th><th>操作</th></tr></thead><tbody>
{foreach from=$CA_IDENTITIES item=identity}<tr><td>{$identity.roster_code|escape:'html'}</td><td>{$identity.real_name|escape:'html'}</td><td>{$identity.formal_seat_state|escape:'html'}</td><td>{$identity.family_used|escape:'html'} / {$identity.family_total|escape:'html'}</td><td>{if $identity.anonymous_state}{$identity.anonymous_state|escape:'html'}{else}未配置{/if}</td><td>{$identity.state|escape:'html'}</td><td>{if $identity.registered_at}{$identity.registered_at|escape:'html'}{else}未认领{/if}</td><td>{if $identity.last_activity}{$identity.last_activity|escape:'html'}{else}—{/if}</td><td><a href="{$CA_BASE_URL|escape:'html'}identities&amp;identity_id={$identity.id|escape:'url'}">查看</a></td></tr>{foreachelse}<tr><td colspan="9">尚未建立同学身份。</td></tr>{/foreach}
