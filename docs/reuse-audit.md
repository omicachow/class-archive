...
| Thumbnail, preview and original tiers | Core derivative generation/cache plus ClassArchivePolicy MediaGuard and nginx internal delivery | **Yes for the final localhost baseline** | Final Theme representative/format choices and browser/touch UX still need QA; active Family-account release remains a lifecycle gate | Maintain the thin server-side MediaGuard and same-size safe-preview fallback; do not replace Core generation or stream 200GB through PHP | Final 290 matrix + 38 mutable/path-alias + 16 small-photo + 75 Pending-state probes pass and restore 72 images |
...
| Private gallery and no open signup | Core config, private albums/groups + MediaGuard + ClassIdentity Claim provisioning | Yes for current read/registration paths | Active Family release, member password reset and future endpoints are not complete | Keep the thin ClassIdentity lifecycle adapter and MediaGuard at every media path | Guest UI/API/media are denied; open registration remains disabled; one-time Classmate/Teacher Claims are the supported account-creation path |
| HERITAGE/LIVING read isolation | Private root albums + group ACL/inheritance + effective-Era MediaGuard | **Yes for current album/API/media paths** | Upload, collections and future endpoints still need their own action policy; active Family release is pending | Keep one central MediaGuard resolver backed by explicit ClassIdentity Principals and negative endpoint tests | Family gets HERITAGE preview only; Classmate/Teacher/Anonymous get permitted both-era previews; freeze/session/Core ACL revocation pass and a cross-Era association denies every role |
...
| Family Pending -> Admin Approve | Community 16.f moderation | Potentially; workflow demonstrated in evaluation | Supported runtime keeps it inactive; moderation endpoint accepted tokenless POST; MediaGuard Pending-state 75-GET focused gate passes but upload/review actions do not | Guarded moderation action/audit; no new uploader or blob store | Reuse only after CSRF/input/upload-mode gates; the media-state guard alone does not enable submission |
...
| Anonymous Seat account | Core account + ClassIdentity Seat/Principal/CapabilityGuard | **Implemented foundation** | Admin governance page and enable/disable action are pending | Maintain one independent account per Anonymous Seat and the fail-closed action guard | Hidden profile/discovery, no upload/album/rating/favorite, independent credentials and admin trace are verified |
| Context-scoped anonymous pseudonym | ClassIdentity AnonymousPresenter and resolution service | **Implemented foundation** | Production comment enablement, reports and later album contexts remain pending | Maintain HMAC alias derivation plus HTML/API output filtering and audited admin resolution | Same-context stability, cross-context unlinkability, redaction and 211 real HTTP assertions pass |
...
Principal/lifecycle/Admin/CapabilityGuard/AnonymousPresenter services,
`ClassArchivePolicy` with MediaGuard and later archive/submission/collection
policy, `ClassSpotlight`, a small Like/Report adapter if retained, and the
...
and business pages, persisted MediaGuard HTTP attestation, Admin MFA,
