# HumHub comparison instance: first local installation
...
The ignored `.env` is part of the recovery set because it contains the credentials and keyed-derivation secrets matching the persistent volumes. Keep an encrypted copy separately from the database and uploads. Never use `init-dev-env.ps1 -Force` against an initialized database volume; the script refuses that operation.
...
Do not disable or uninstall Gallery to troubleshoot it. Gallery 1.7.1 removes Gallery/Media records during uninstall; any lifecycle operation requires a database backup, upload-volume backup, and a proven restore first.
